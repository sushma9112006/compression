  ...
  while (position < data.length) {
    ...
    for (let i = start; i < position; i++) {
      ...
      if (j > bestMatch.length) {
        bestMatch = { distance: position - i, length: j };
      }
    }
  ...
  while (position < compressedData.length) {
    ...
    if (flag === 0) {
      ...
      if (distance > output.length || distance <= 0) {
        throw new Error(`Invalid distance value: ${distance}`); // Fixed
      }
      ...
      throw new Error(`Invalid flag byte: ${flag}`); // Fixed
    }
  }
  return new Uint8Array(output);
}
...
Processing Time: ${formatTime(processingTime)}`; // Fixed
  statsEl.style.display = "block";
}

function updateTimeDisplay() {
  timeValue.textContent = `Processing time: ${formatTime(processingTime)}`; // Fixed
...
compressDeflateBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    processedBlob = new Blob([compressed], { type: "application/octet-stream" });
...
decompressBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    processedBlob = new Blob([decompressed.originalData], { type: "application/octet-stream" });
